my_data=read.csv("C:/Users/shubhamv/Downloads/bankloan.csv")
dim(my_data)
str(my_data)
...
library(ROSE) # Random Over-Sampling Ex- amples
set.seed(123)
my_tree_data= ovun.sample(default ~ ., data = my_data, method = "over",N =1034)$data
...
table(pred_test,test_data1$default)
...
table(pred_train,train_data1$default)
...
table(prune_test,test_data1$default)
...
table(prune_train,train_data1$default)
...
table(bag_test_predict,test_data1$default)
...
table(bag_train_predict,train_data1$default)
...
table(random_forest_predicted,test_data1$default)
